...
class Hyperparameters:
    ...
    leaky_relu_negative_slope: float = float(os.environ.get("LEAKY_RELU_NEGATIVE_SLOPE", 0.0))
...
class MLP(nn.Module):
    # LeakyReLU^2 MLP. Keep negative_slope=0.0 to recover the original relu^2 baseline.
    ...
    def __call__(self, x: mx.array) -> mx.array:
        x = nn.leaky_relu(self.fc(x), negative_slope=self.negative_slope)
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult, negative_slope=negative_slope)
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, negative_slope)
...
def main() -> None:
    ...
    log(f"leaky_relu_negative_slope:{args.leaky_relu_negative_slope}")
