...
class Hyperparameters:
    ...
    xsa_last_n: int = int(os.environ.get("XSA_LAST_N", 6))
    ...
    attn_gate_init: float = float(os.environ.get("ATTN_GATE_INIT", 1.0))
    value_residual_init: float = float(os.environ.get("VALUE_RESIDUAL_INIT", 0.0))
    attn_gate_enable: bool = bool(int(os.environ.get("ATTN_GATE_ENABLE", "1")))
    value_residual_enable: bool = bool(int(os.environ.get("VALUE_RESIDUAL_ENABLE", "1")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,attn_gate,lambda_v,skip_weight,skip_weights",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        attn_gate_init: float,
        attn_gate_enable: bool,
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.attn_gate = mx.ones((num_heads,), dtype=mx.float32) * attn_gate_init
        self.attn_gate_enable = attn_gate_enable
        self.rope = nn.RoPE(self.head_dim, traditional=False, base=rope_base)
        self.scale = self.head_dim**-0.5
        self.use_xsa = False

    def _xsa_efficient(self, y: mx.array, v: mx.array) -> mx.array:
        # GQA-aware XSA helper: reshape into KV-head groups instead of repeating values.
        bsz, seqlen, num_heads, head_dim = y.shape
        num_kv_heads = v.shape[2]
        group_size = num_heads // num_kv_heads
        y_grouped = y.reshape(bsz, seqlen, num_kv_heads, group_size, head_dim)
        v_sq = mx.sum(mx.square(v.astype(mx.float32)), axis=-1, keepdims=True)
        v_norm = v.astype(mx.float32) / mx.sqrt(v_sq + 1e-6)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.use_xsa:
            y = self._xsa_efficient(y, v.transpose(0, 2, 1, 3))
        if self.attn_gate_enable:
            y = y * self.attn_gate.astype(y.dtype)[None, None, :, None]
...
class Block(nn.Module):
    def __init__(
        ...
        attn_gate_enable: bool,
        value_residual_enable: bool,
    ):
        super().__init__()
        self.attn_norm = RMSNormNoWeight()
        self.mlp_norm = RMSNormNoWeight()
        self.attn = CausalSelfAttention(
            dim, num_heads, num_kv_heads, rope_base, qk_gain_init, attn_gate_init, attn_gate_enable
        )
        ...
        self.lambda_v = mx.array(value_residual_init, dtype=mx.float32)
        self.value_residual_enable = value_residual_enable
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        if self.value_residual_enable:
            x = x + self.lambda_v.astype(x.dtype) * x0
    ...
    def __init__(
        ...
        self.blocks = [
            Block(
                ...
                value_residual_init,
                attn_gate_enable,
                value_residual_enable,
            )
            for i in range(num_layers)
        ]
        self.final_norm = RMSNormNoWeight()
        if xsa_last_n > 0:
            active_start = max(0, num_layers - min(num_layers, xsa_last_n))
            for i in range(active_start, num_layers):
...
def main() -> None:
    ...
    model = GPT(
        ...
        attn_gate_enable=args.attn_gate_enable,
        value_residual_enable=args.value_residual_enable,
    )
    ...
    log(f"xsa_last_n:{args.xsa_last_n}")
    log(
        f"pr824_mimic attn_gate_init:{args.attn_gate_init} "
        f"value_residual_init:{args.value_residual_init} "
        f"attn_gate_enable:{args.attn_gate_enable} "
        f"value_residual_enable:{args.value_residual_enable}"
    )
    ...
    if args.xsa_last_n > 0:
        xsa_layers = [i for i, b in enumerate(model.blocks) if getattr(b.attn, "use_xsa", False)]
        log(f"xsa_last_n:{args.xsa_last_n} active_layers:{xsa_layers}")
