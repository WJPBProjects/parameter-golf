...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 6))
    ...
    attn_gate_init = float(os.environ.get("ATTN_GATE_INIT", 1.0))
    value_residual_init = float(os.environ.get("VALUE_RESIDUAL_INIT", 0.0))
    attn_gate_enable = bool(int(os.environ.get("ATTN_GATE_ENABLE", "1")))
    value_residual_enable = bool(int(os.environ.get("VALUE_RESIDUAL_ENABLE", "1")))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.attn_gate = nn.Parameter(torch.full((num_heads,), attn_gate_init, dtype=torch.float32))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.attn_gate_enable:
            y = y * self.attn_gate.to(dtype=y.dtype)[None, None, :, None]
...
class Block(nn.Module):
    def __init__(
        ...
        self.lambda_v = nn.Parameter(torch.tensor(value_residual_init, dtype=torch.float32))
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        if self.value_residual_enable:
            x = x + self.lambda_v.to(dtype=x.dtype) * x0
...
def main() -> None:
    ...
    if args.xsa_last_n > 0:
        xsa_layers = [i for i, b in enumerate(base_model.blocks) if getattr(b.attn, "use_xsa", False)]
