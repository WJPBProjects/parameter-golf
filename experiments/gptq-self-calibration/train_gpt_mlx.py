...
class Hyperparameters:
    ...
    temp_scaling: bool = bool(int(os.environ.get("TEMP_SCALING", "0")))
    calibration_source: str = os.environ.get("CALIBRATION_SOURCE", "validation").strip().lower()
    calibration_tokens: int = int(os.environ.get("CALIBRATION_TOKENS", 2048))
    calibration_prompt_tokens: int = int(os.environ.get("CALIBRATION_PROMPT_TOKENS", 64))
    calibration_temps: tuple[float, ...] = tuple(
        float(x)
        for x in os.environ.get("CALIBRATION_TEMPS", "0.90,0.95,1.00,1.05,1.10").split(",")
        if x.strip()
    )
...
class GPT(nn.Module):
    ...
    def logits(self, input_ids: mx.array) -> mx.array:
        ...
        return logits.reshape(*input_ids.shape, self.tok_emb.weight.shape[0])
    ...
    def loss(
        self, input_ids: mx.array, target_ids: mx.array, temperature: float = 1.0
    ) -> mx.array:
        ...
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ self.tok_emb.weight.astype(x.dtype).T
            logits = self.softcap(logits_proj) / temperature
...
class Muon:
    ...
    def step(
        self, params: dict[str, mx.array], grads: dict[str, mx.array], step: int, lr_mul: float
    ) -> dict[str, mx.array]:
...
def validate_dataset_tokenizer_pair(
    ...
    dataset_entry = next(
        (x for x in manifest.get("datasets", []) if x.get("name") == dataset_dir.name), None
    )
    ...
    expected_name = Path(
        (tokenizer_entry or {}).get("model_path") or (tokenizer_entry or {}).get("path") or ""
    ).name
...
def build_random_calibration_tokens(
    vocab_size: int, total_tokens: int, seed: int
) -> np.ndarray:
    rng = np.random.default_rng(seed)
    tokens = rng.integers(0, vocab_size, size=total_tokens, dtype=np.int32)
    return np.ascontiguousarray(tokens)


def build_self_generated_calibration_tokens(
    args: Hyperparameters,
    model: GPT,
    train_loader: TokenLoader,
    total_tokens: int,
) -> np.ndarray:
    prompt_tokens = max(1, min(args.calibration_prompt_tokens, total_tokens))
    prompt = np.asarray(train_loader.stream.take(prompt_tokens), dtype=np.int32)
    generated = prompt.tolist()
    while len(generated) < total_tokens:
        context = np.asarray(generated[-args.train_seq_len :], dtype=np.int32)
        input_ids = mx.array(context[None, :], dtype=mx.int32)
        logits = model.logits(input_ids)
        next_token = int(mx.argmax(logits[0, -1]).item())
        generated.append(next_token)
    return np.ascontiguousarray(np.asarray(generated[:total_tokens], dtype=np.int32))
...
def build_calibration_tokens(
    ...
    total_tokens = max(args.calibration_tokens, args.train_seq_len + 1)
    if source == "validation":
        return np.ascontiguousarray(val_tokens[: min(val_tokens.size, total_tokens + 1)])
    if source == "random_tokens":
        return build_random_calibration_tokens(args.vocab_size, total_tokens + 1, args.seed)
    if source == "self_generated":
        return build_self_generated_calibration_tokens(args, model, train_loader, total_tokens + 1)
    raise ValueError(
        "CALIBRATION_SOURCE must be one of: validation, random_tokens, self_generated; "
        f"got {args.calibration_source!r}"
    )
...
def loss_and_grad_chunked(
    ...
    for chunk_tokens in chunk_sizes:
        ...
        loss, grads = compiled_loss_and_grad(x, y, 1.0)
...
def eval_val(
    ...
    for batch_idx, batch_seq_start in enumerate(range(0, total_seqs, val_batch_seqs), start=1):
        ...
        batch_loss = compiled_loss(x, y, temperature).astype(mx.float32)
        mx.eval(batch_loss)
        total_loss_sum += float(batch_loss.item()) * chunk_token_count
        prev_ids = x_np.reshape(-1)
        tgt_ids = y_np.reshape(-1)
        bytes_np = base_bytes_lut[tgt_ids].astype(np.int16, copy=True)
        bytes_np += (has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]).astype(
            np.int16, copy=False
        )
...
def find_temperature(
    ...
    best_t = 1.0
    best_loss = float("inf")
    for temperature in args.calibration_temps:
        loss, _ = eval_val(
            args,
            lambda x, y, t: model.loss(x, y, temperature=t),
            calibration_tokens,
            base_bytes_lut,
...
def main() -> None:
    ...
    compiled_loss = mx.compile(
        lambda x, y, temperature: model.loss(x, y, temperature=temperature),
        inputs=model.state,
        outputs=model.state,
    )
    compiled_loss_and_grad = mx.compile(
        nn.value_and_grad(
            model, lambda x, y, temperature: model.loss(x, y, temperature=temperature)
        ),
        inputs=model.state,
        outputs=model.state,
    )
    ...
    if args.temp_scaling:
        log(
            f"temp_scaling:enabled calibration_source:{args.calibration_source} "
            f"calibration_tokens:{args.calibration_tokens} calibration_prompt_tokens:{args.calibration_prompt_tokens} "
            f"calibration_temps:{','.join(f'{t:.2f}' for t in args.calibration_temps)}"
        )
    log(
    ...
    if args.warmup_steps > 0:
        ...
        warm_val_seqs = min(
            val_batch_tokens // args.train_seq_len, (val_tokens.size - 1) // args.train_seq_len
        )
        ...
        warm_val_loss = compiled_loss(x_val, y_val, 1.0)
    ...
    temperature = 1.0
    if args.temp_scaling:
        calibration_tokens = build_calibration_tokens(args, model, train_loader, val_tokens)
        ...
        log(
            f"temp_scaling calibration_source:{args.calibration_source} "
            f"temperature:{temperature:.2f} calibration_tokens:{calibration_tokens.size - 1}"
        )
