...
class Hyperparameters:
    ...
    value_scale_enabled = bool(int(os.environ.get("VALUE_SCALE_ENABLED", "0")))
    value_scale_last_n = int(os.environ.get("VALUE_SCALE_LAST_N", 3))
    value_scale_init = float(os.environ.get("VALUE_SCALE_INIT", 1.0))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if value_scale_enabled:
            self.value_scale = nn.Parameter(
                torch.full((self.num_kv_heads,), value_scale_init, dtype=torch.float32)
            )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.value_scale_enabled:
            v = v * self.value_scale.to(dtype=v.dtype)[None, :, None, None]
