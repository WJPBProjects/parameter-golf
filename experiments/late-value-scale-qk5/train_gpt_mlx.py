...
class Hyperparameters:
    ...
    value_scale_enabled: bool = bool(int(os.environ.get("VALUE_SCALE_ENABLED", "0")))
    value_scale_last_n: int = int(os.environ.get("VALUE_SCALE_LAST_N", 3))
    value_scale_init: float = float(os.environ.get("VALUE_SCALE_INIT", 1.0))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,value_scale",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.value_scale_enabled = value_scale_enabled
        if value_scale_enabled:
            self.value_scale = mx.ones((self.num_kv_heads,), dtype=mx.float32) * value_scale_init
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.value_scale_enabled:
            v = v * self.value_scale.astype(v.dtype)[None, :, None, None]
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        qk_gain_init: float,
        value_scale_enabled: bool,
        value_scale_last_n: int,
        value_scale_init: float,
    ):
        super().__init__()
        if logit_softcap <= 0.0:
            raise ValueError(f"logit_softcap must be positive, got {logit_softcap}")
        if value_scale_last_n < 0:
            raise ValueError(f"value_scale_last_n must be non-negative, got {value_scale_last_n}")
        ...
        self.value_scale_enabled = value_scale_enabled
        self.value_scale_last_n = value_scale_last_n
        self.value_scale_init = value_scale_init
        value_scale_start = max(num_layers - value_scale_last_n, 0)
        self.blocks = [
            Block(
                dim,
                num_heads,
                ...
                rope_base,
                qk_gain_init,
                value_scale_enabled and i >= value_scale_start,
                value_scale_init,
            )
...
def main() -> None:
    ...
    model = GPT(
        ...
        value_scale_enabled=args.value_scale_enabled,
        value_scale_last_n=args.value_scale_last_n,
        value_scale_init=args.value_scale_init,
    )
    ...
    log(
        f"value_scale:enabled={args.value_scale_enabled} "
        f"last_n={args.value_scale_last_n} init={args.value_scale_init}"
    )
