...
class CastedLinear(nn.Linear):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Tensor-backed toggle so `torch.compile` cannot freeze late-QAT off as a Python constant.
        self.register_buffer("qat_enabled", torch.zeros((), dtype=torch.float32), persistent=False)

    def set_qat_enabled(self, enabled: bool) -> None:
        self.qat_enabled.fill_(1.0 if enabled else 0.0)

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self.training and w.ndim == 2:
            # Fake-quantize the forward path only when the tensor toggle is on.
            # The arithmetic gate keeps the branch data-dependent instead of Python-constant.
            w32 = self.weight.float()
            row_max = w32.abs().amax(dim=1)
            scale = (row_max / 127.0).clamp_min(1.0 / 127.0)
            q = torch.clamp(torch.round(w32 / scale[:, None]), -127, 127)
            w_q = (q * scale[:, None]).to(dtype=x.dtype)
            qat_gate = self.qat_enabled.to(dtype=x.dtype)
            w = w + qat_gate * (w_q - w).detach()
...
def set_qat_enabled(module: nn.Module, enabled: bool) -> None:
    for submodule in module.modules():
        if isinstance(submodule, CastedLinear):
            submodule.set_qat_enabled(enabled)


def qat_is_enabled(module: nn.Module) -> bool:
    for submodule in module.modules():
        if isinstance(submodule, CastedLinear) and bool(submodule.qat_enabled.item()):
            return True
    return False
...
def main() -> None:
    ...
    set_qat_enabled(base_model, args.qat_enabled)
    ...
    log0(
        f"qat_enabled:{args.qat_enabled} late_qat_threshold:{args.late_qat_threshold:.4f}"
    )
    ...
    while True:
        ...
        if (
            ...
            and not qat_is_enabled(base_model)
        ):
