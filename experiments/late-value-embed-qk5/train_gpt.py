...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,ve_layer_scale,ve_layer_scales,ve_shared.scale",
    ).split(",")
    if pattern
)
...
class ValueEmbedding(nn.Module):
    # Shared token-identity reinjection path for selected late attention layers.
    def __init__(self, vocab_size: int, ve_dim: int, target_dim: int):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, ve_dim)
        nn.init.normal_(self.embed.weight, mean=0.0, std=0.01)
        self.proj = CastedLinear(ve_dim, target_dim, bias=False) if ve_dim != target_dim else None
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[block_idx](
                x,
                x0,
                v_embed=self._get_ve(block_idx, input_ids, ve_cache),
            )
...
def main() -> None:
    ...
    if base_model.ve_shared is not None:
        tok_param_groups.append(
            {"params": [base_model.ve_shared.embed.weight], "lr": token_lr, "base_lr": token_lr}
        )
    ...
    log0(f"value_embedding:enabled={args.ve_enabled} dim:{args.ve_dim} layers:{args.ve_layers}")
