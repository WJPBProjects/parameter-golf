...
class Hyperparameters:
    ...
    parallel_residual: bool = bool(int(os.environ.get("PARALLEL_RESIDUAL", "1")))
    parallel_start_layer: int = int(os.environ.get("PARALLEL_START_LAYER", 6))
...
class Block(nn.Module):
    def __init__(
        ...
        if parallel_residual:
            self.attn_to_attn = mx.ones((dim,), dtype=mx.float32)
            self.attn_to_mlp = mx.zeros((dim,), dtype=mx.float32)
            self.mlp_to_attn = mx.zeros((dim,), dtype=mx.float32)
            self.mlp_to_mlp = mx.ones((dim,), dtype=mx.float32)
    ...
    def parallel_step(
        ...
        if not self.parallel_residual:
            raise ValueError("parallel_step called on a non-parallel block")
        mix = self.resid_mix.astype(x_attn.dtype)
        x_attn_in = mix[0][None, None, :] * x_attn + mix[1][None, None, :] * x0
        x_mlp_in = mix[0][None, None, :] * x_mlp + mix[1][None, None, :] * x0
        attn_out = self.attn_scale.astype(x_attn.dtype)[None, None, :] * self.attn(
            self.attn_norm(x_attn_in)
        )
        mlp_out = self.mlp_scale.astype(x_attn.dtype)[None, None, :] * self.mlp(
            self.mlp_norm(x_mlp_in)
        )
        next_attn = (
            x_attn
            + self.attn_to_attn.astype(x_attn.dtype)[None, None, :] * attn_out
            + self.mlp_to_attn.astype(x_attn.dtype)[None, None, :] * mlp_out
        )
        next_mlp = (
            x_mlp
            + self.attn_to_mlp.astype(x_attn.dtype)[None, None, :] * attn_out
            + self.mlp_to_mlp.astype(x_attn.dtype)[None, None, :] * mlp_out
        )
        return next_attn, next_mlp
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.parallel_residual = parallel_residual
        self.parallel_start_layer = max(parallel_start_layer, 0)
        ...
        self.blocks = [
            Block(
                ...
                rope_base,
                qk_gain_init,
                parallel_residual=parallel_residual and i >= self.parallel_start_layer,
            )
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        x0 = x
        skips: list[mx.array] = []
        x_attn: mx.array | None = None
        x_mlp: mx.array | None = None

        for i in range(self.num_encoder_layers):
            block = self.blocks[i]
            if self.parallel_residual and i >= self.parallel_start_layer:
                if x_attn is None or x_mlp is None:
                    x_attn = x
                    x_mlp = x
                x_attn, x_mlp = block.parallel_step(x_attn, x_mlp, x0)
                x = (x_attn + x_mlp) * 0.5
            else:
                x = block(x, x0)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                skip = self.skip_weights[i].astype(x.dtype)[None, None, :] * skips.pop()
                x = x + skip
                if x_attn is not None and x_mlp is not None:
                    x_attn = x_attn + skip
                    x_mlp = x_mlp + skip
            layer_idx = self.num_encoder_layers + i
            block = self.blocks[layer_idx]
            if self.parallel_residual and layer_idx >= self.parallel_start_layer:
                if x_attn is None or x_mlp is None:
                    x_attn = x
                    x_mlp = x
                x_attn, x_mlp = block.parallel_step(x_attn, x_mlp, x0)
...
def main() -> None:
    ...
    log(
        f"parallel_residual:{args.parallel_residual} parallel_start_layer:{args.parallel_start_layer}"
    )
    log(
