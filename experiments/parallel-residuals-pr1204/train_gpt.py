...
class Block(nn.Module):
    ...
    def parallel_step(self, x_attn: Tensor, x_mlp: Tensor, x0: Tensor) -> tuple[Tensor, Tensor]:
        if not self.parallel_residual:
            raise RuntimeError("parallel_step called on a non-parallel block")
        mix = self.resid_mix.to(dtype=x_attn.dtype)
        x_attn_in = mix[0][None, None, :] * x_attn + mix[1][None, None, :] * x0
        x_mlp_in = mix[0][None, None, :] * x_mlp + mix[1][None, None, :] * x0
        attn_out = self.attn_scale.to(dtype=x_attn.dtype)[None, None, :] * self.attn(
        ...
        next_attn = (
            x_attn
            + self.attn_to_attn.to(dtype=x_attn.dtype)[None, None, :] * attn_out
            + self.mlp_to_attn.to(dtype=x_attn.dtype)[None, None, :] * mlp_out
        )
        next_mlp = (
            x_mlp
            + self.attn_to_mlp.to(dtype=x_attn.dtype)[None, None, :] * attn_out
            + self.mlp_to_mlp.to(dtype=x_attn.dtype)[None, None, :] * mlp_out
        )
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x0 = x
        skips: list[Tensor] = []
        x_attn: Tensor | None = None
        x_mlp: Tensor | None = None
