...
class Hyperparameters:
    ...
    spline_layer_mode: str = os.environ.get("SPLINE_LAYER_MODE", "decoder")
    spline_radius: int = int(os.environ.get("SPLINE_RADIUS", 4))
    spline_num_knots: int = int(os.environ.get("SPLINE_NUM_KNOTS", 4))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "spline_kernel,spline_kernels,spline_scale,spline_scales",
    ).split(",")
    if pattern
)
...
def causal_shift(x: mx.array, shift: int) -> mx.array:
    if shift <= 0:
        return x
    zeros = mx.zeros((x.shape[0], shift, x.shape[2]), dtype=x.dtype)
    return mx.concatenate((zeros, x[:, : x.shape[1] - shift, :]), axis=1)


class SplineMix(nn.Module):
    # Spline-inspired local token mixer: smooth learnable distance weights over a
    # short causal window.
    def __init__(self, dim: int, radius: int, num_knots: int):
        super().__init__()
        if radius < 0:
            raise ValueError("spline radius must be non-negative")
        if num_knots < 2:
            raise ValueError("spline num_knots must be at least 2")
        self.radius = radius
        self.num_knots = num_knots
        self.knots = mx.linspace(0.0, 1.0, num_knots)
        self.kernel = mx.random.normal((num_knots, dim), dtype=mx.float32) * 0.01

    def __call__(self, x: mx.array) -> mx.array:
        if self.radius == 0:
            return mx.zeros_like(x)
        out = mx.zeros_like(x)
        denom = float(max(self.radius, 1))
        knots = self.knots.astype(x.dtype)
        kernel = self.kernel.astype(x.dtype)
        for shift in range(self.radius + 1):
            u = float(shift) / denom
            basis = mx.maximum(1.0 - mx.abs(u - knots) * self.num_knots, 0.0)
            weight = basis @ kernel
            out = out + causal_shift(x, shift) * weight[None, None, :]
        return out
...
class Block(nn.Module):
    def __init__(
        ...
        self.spline_norm = RMSNormNoWeight() if use_spline else None
        self.spline = (
            SplineMix(dim, spline_radius, spline_num_knots) if use_spline else None
        )
        self.attn_scale = mx.ones((dim,), dtype=mx.float32)
        self.mlp_scale = mx.ones((dim,), dtype=mx.float32)
        self.spline_scale = mx.zeros((dim,), dtype=mx.float32)
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        if self.use_spline and self.spline is not None and self.spline_norm is not None:
            spline_out = self.spline(self.spline_norm(x))
            x = x + self.spline_scale.astype(x.dtype)[None, None, :] * spline_out
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        spline_layer_mode = spline_layer_mode.lower()
        if spline_layer_mode not in {"none", "decoder", "all", "last4"}:
            raise ValueError(
                "spline_layer_mode must be one of: none, decoder, all, last4"
            )
        ...
        self.blocks = [
            Block(
                ...
                rope_base,
                qk_gain_init,
                spline_layer_mode == "all"
                or (spline_layer_mode == "decoder" and i >= self.num_encoder_layers)
                or (spline_layer_mode == "last4" and i >= num_layers - 4),
                spline_radius,
                spline_num_knots,
            )
...
def main() -> None:
    ...
    model = GPT(
        ...
        spline_layer_mode=args.spline_layer_mode,
        spline_radius=args.spline_radius,
        spline_num_knots=args.spline_num_knots,
    )
    ...
    log(
        f"spline_hybrid:mode:{args.spline_layer_mode} radius:{args.spline_radius} "
        f"knots:{args.spline_num_knots}"
    )
    log(
