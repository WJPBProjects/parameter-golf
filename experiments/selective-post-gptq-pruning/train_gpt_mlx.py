...
class Hyperparameters:
    ...
    post_gptq_prune_fraction: float = float(os.environ.get("POST_GPTQ_PRUNE_FRACTION", 0.0))
    post_gptq_prune_min_numel: int = int(os.environ.get("POST_GPTQ_PRUNE_MIN_NUMEL", 16_384))
...
def prune_quantized_payload_int8(
    quantized: dict[str, np.ndarray],
    prune_fraction: float,
    min_numel: int,
    stats: dict[str, int],
) -> None:
    prune_fraction = min(max(float(prune_fraction), 0.0), 0.5)
    if prune_fraction <= 0.0:
        return
    min_numel = max(int(min_numel), 1)
    for name, q in quantized.items():
        if q.ndim != 2 or q.size < min_numel:
            continue
        flat_abs = np.abs(np.asarray(q, dtype=np.float32)).reshape(-1)
        nonzero = flat_abs[flat_abs > 0]
        if nonzero.size == 0:
            continue
        threshold = float(np.quantile(nonzero, prune_fraction))
        mask = flat_abs <= threshold
        pruned = int(np.count_nonzero(mask))
        if pruned > 0:
            q.reshape(-1)[mask] = 0
            stats["pruned_int8_tensors"] += 1
            stats["pruned_int8_entries"] += pruned
...
def main() -> None:
    ...
    prune_quantized_payload_int8(
        quant_obj["quantized"],
        args.post_gptq_prune_fraction,
        args.post_gptq_prune_min_numel,
        quant_stats,
    )
    ...
    if args.post_gptq_prune_fraction > 0.0:
        log(
            "post_gptq_pruning "
            f"fraction:{args.post_gptq_prune_fraction:.4f} "
            f"min_numel:{args.post_gptq_prune_min_numel} "
            f"pruned_tensors:{quant_stats['pruned_int8_tensors']} "
            f"pruned_entries:{quant_stats['pruned_int8_entries']}"
        )
