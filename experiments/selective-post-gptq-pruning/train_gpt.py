...
class Hyperparameters:
    ...
    post_gptq_prune_fraction = float(os.environ.get("POST_GPTQ_PRUNE_FRACTION", 0.0))
    post_gptq_prune_min_numel = int(os.environ.get("POST_GPTQ_PRUNE_MIN_NUMEL", 16_384))
...
def prune_quantized_payload_int8(
    ...
    for name, q in quantized.items():
        if q.ndim != 2 or q.numel() < min_numel:
            continue
        flat_abs = q.abs().to(dtype=torch.float32).flatten()
        nonzero = flat_abs[flat_abs > 0]
        if nonzero.numel() == 0:
            continue
        threshold = float(torch.quantile(nonzero, prune_fraction).item())
