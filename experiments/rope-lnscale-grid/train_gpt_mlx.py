...
class Hyperparameters:
    ...
    rope_dim: int = int(os.environ.get("ROPE_DIM", 0))
    ln_scale_init: str = os.environ.get("LN_SCALE_INIT", "ones")
...
def layer_scale_init_value(layer_idx: int, mode: str) -> float:
    mode = mode.strip().lower()
    if mode in {"", "none", "ones", "one"}:
        return 1.0
    depth = float(layer_idx + 1)
    if mode in {"inv_sqrt", "sqrt", "1/sqrt", "1_over_sqrt"}:
        return 1.0 / math.sqrt(depth)
    if mode in {"inv_linear", "linear", "1/l", "1_over_l"}:
        return 1.0 / depth
    raise ValueError(f"LN_SCALE_INIT must be one of ones|inv_sqrt|inv_linear, got {mode!r}")
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.rope_dim = self.head_dim if rope_dim <= 0 else rope_dim
        if self.rope_dim > self.head_dim:
            raise ValueError(f"ROPE_DIM must be <= head_dim ({self.head_dim}), got {self.rope_dim}")
        if self.rope_dim % 2 != 0:
            raise ValueError("ROPE_DIM must be even for RoPE")
        ...
        self.rope = nn.RoPE(self.rope_dim, traditional=False, base=rope_base)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        q_rot = self.rope(q[..., : self.rope_dim]) if self.rope_dim < self.head_dim else self.rope(q)
        k_rot = self.rope(k[..., : self.rope_dim]) if self.rope_dim < self.head_dim else self.rope(k)
        if self.rope_dim < self.head_dim:
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            dim, num_heads, num_kv_heads, rope_base, rope_dim, qk_gain_init
        )
        self.mlp = MLP(dim, mlp_mult)
        self.attn_scale = mx.ones((dim,), dtype=mx.float32) * attn_scale_init
        self.mlp_scale = mx.ones((dim,), dtype=mx.float32) * mlp_scale_init
    ...
    def __init__(
        ...
        self.blocks = [
            Block(
                ...
                rope_dim,
                qk_gain_init,
                layer_scale_init_value(i, ln_scale_init),
                layer_scale_init_value(i, ln_scale_init),
            )
...
def main() -> None:
    ...
    model = GPT(
        ...
        ln_scale_init=args.ln_scale_init,
    ...
    log(
        ...
        f"rope_dim:{args.rope_dim if args.rope_dim > 0 else args.model_dim // args.num_heads} "
        f"ln_scale_init:{args.ln_scale_init}"
    )
    log(
