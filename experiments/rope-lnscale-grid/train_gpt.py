...
class Hyperparameters:
    ...
    rope_dim = int(os.environ.get("ROPE_DIM", 0))
    ln_scale_init = os.environ.get("LN_SCALE_INIT", "ones")
...
def apply_partial_rotary_emb(x: Tensor, cos: Tensor, sin: Tensor, rope_dim: int) -> Tensor:
    if rope_dim <= 0 or rope_dim >= x.size(-1):
        return apply_rotary_emb(x, cos, sin)
    rotary = apply_rotary_emb(x[..., :rope_dim], cos, sin)
