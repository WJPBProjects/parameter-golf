...
class Hyperparameters:
    ...
    smear_last_n: int = int(os.environ.get("SMEAR_LAST_N", 3))
    smear_init: float = float(os.environ.get("SMEAR_INIT", 0.0))
...
class SmearGate(nn.Module):
    """Add a tiny previous-token shortcut in latent space."""

    def __init__(self, dim: int, smear_init: float):
        super().__init__()
        self.gate = mx.full((dim,), smear_init, dtype=mx.float32)

    def __call__(self, x: mx.array) -> mx.array:
        x_prev = mx.concatenate([mx.zeros_like(x[:, :1]), x[:, :-1]], axis=1)
        gate = self.gate.astype(x.dtype)[None, None, :]
        return x + gate * (x_prev - x)
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        if smear_last_n < 0:
            raise ValueError(f"smear_last_n must be non-negative, got {smear_last_n}")
        self.logit_chunk_tokens = logit_chunk_tokens
        self.logit_softcap = logit_softcap
        self.smear_enabled = smear_enabled
        self.smear_start = max(num_layers - smear_last_n, 0)
        ...
        self.smear = SmearGate(dim, smear_init) if smear_enabled else None
        self.blocks = [
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if self.smear is not None and i >= self.smear_start:
                x = self.smear(x)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            layer_index = self.num_encoder_layers + i
            x = self.blocks[layer_index](x, x0)
            if self.smear is not None and layer_index >= self.smear_start:
                x = self.smear(x)
...
def main() -> None:
    ...
    model = GPT(
        ...
        smear_last_n=args.smear_last_n,
    ...
    log(
        f"smear:enabled={args.smear_enabled} last_n={args.smear_last_n} init={args.smear_init}"
    )
    log(
