...
class Hyperparameters:
    ...
    smear_last_n = int(os.environ.get("SMEAR_LAST_N", 3))
...
class SmearGate(nn.Module):
    ...
    def __init__(self, dim: int, smear_init: float):
        super().__init__()
        self.gate = nn.Parameter(torch.full((dim,), smear_init, dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        x_prev = torch.cat([torch.zeros_like(x[:, :1]), x[:, :-1]], dim=1)
        gate = self.gate.to(dtype=x.dtype)[None, None, :]
